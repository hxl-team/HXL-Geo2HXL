...
$countryName = "Burkina Faso" ; //Only used when processing Admin 0, otherwise ignored.
$countryPcode = "BFA" ; //Only used when processing Admin 0 and Admin 1, otherwise ignored.  This means that the pcode for the country itself does not need to be included in the Admin0 or Admin1 CSV input.  For lower admin levels (admin > 1), the pcode of the level immediately above must be included.

$country_iso3 = "bfa" ; //ISO 3166 Alpha3 three letter code for the country, lower case.  This becomes part of the URI for the features generated by the script.
...
$level_n_minus_one_pcode_element = 5 ;  //which column contains the pcode for the admin unit one level above the level that is being converted. This is ignored if $n = 0 or $n = 1 (See below).
$n = 0 ; //base admin level being processed. Set to 0 if you are processing the national boundary. Set to 999 if you are processing the populated places layer.  Otherwise set to the admin level you are processing: 1, 2, 3, etc.
$featureName_element = 9 ; //which column contains the feature name. Ignored if $n = 0.
$featureRefName_element = 9 ; //which column contains the feature ref name.  Note that the script will automatically change accented characters if found, so this can be the same column as the $featureName_element. Ignored if $n = 0.
...
//Metadata items
$dcdate = "2012-11-20T11:16:00.0Z" ; //the date the file is created.  Format must be ISO 8601 format (level of granularity below the day is optional).
$validon = "2012-11-20" ;  //Beginning date for which this dataset is the valid one (in ISO 8601 format, level of granularity below the day is optional). This value is applied to the data container (i.e. named graph) which holds the data.  The end of the period of validity for this dataset is the first later ValidOn for a given feature.
