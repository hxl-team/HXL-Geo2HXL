...
$countryName = "Colombia" ; //Only used when processing Admin 0, otherwise ignored.
$countryPcode = "COL" ; //Only used when processing Admin 0 and Admin 1, otherwise ignored.  This means that the pcode for the country itself does not need to be included in the Admin0 or Admin1 CSV input.  For lower admin levels (admin > 1), the pcode of the level immediately above must be included.

$country_iso3 = "col" ; //ISO 3166 Alpha3 three letter code for the country, lower case.  This becomes part of the URI for the features generated by the script.
$geom_element = 0 ; //which column contains the WKT geometry. First column = 0.
$level_n_pcode_element = 3 ; //which column contains the pcode for the level that is being converted.  First column = 0. Ignored if $n = 0 (see below). Set to 999 if the data to convert does not have a p-code. 
...
$n = 2 ; //base admin level being processed. Set to 0 if you are processing the national boundary. Set to 999 if you are processing the populated places layer.  Otherwise set to the admin level you are processing: 1, 2, 3, etc.
$featureName_element = 2 ; //which column contains the feature name. Ignored if $n = 0.
$featureRefName_element = 2 ; //which column contains the feature ref name.  Note that the script will automatically change accented characters if found, so this can be the same column as the $featureName_element. Ignored if $n = 0.
...
$file_to_process  = "/Users/carsten/Desktop/CODs/Colombia - Admin 1 missing/admin_2/ADM_Departamentos.csv" ;
$output_file_name = "/Users/carsten/Desktop/CODs/Colombia - Admin 1 missing/admin_2/ADM_Departamentos.ttl" ;
